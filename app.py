    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark border-bottom border-secondary mb-4">
        <div class="container">
            ...
            <div class="navbar-nav me-auto">
                ...
                <a class="nav-link" href="{{ url_for('posts_feed') }}"><i class="fas fa-newspaper me-1"></i>المنشورات</a>
                <a class="nav-link" href="{{ url_for('profile') }}">الملف الشخصي</a>
                <a class="nav-link text-info" href="{{ url_for('ai_chat') }}"><i class="fas fa-robot me-1"></i>الذكاء الاصطناعي</a>
...
<div class="row">
<div class="col-md-4">
<div class="card bg-dark border-secondary p-4 shadow mb-4 text-center">
{% if user.avatar_url %}
<img src="{{ user.avatar_url }}" alt="صورة الملف" class="rounded-circle mb-3" style="width:120px;height:120px;object-fit:cover;border:3px solid #0d6efd;">
{% else %}
...
{% endif %}
<h4 class="text-primary mb-3">{{ user.username }}</h4>
<p class="mb-1"><strong>البريد:</strong> {{ user.email }}</p>
<p class="mb-3"><strong>الرصيد:</strong> <span class="text-success fw-bold">{{ user.points }} نقطة</span></p>
<form method="POST" action="{{ url_for('update_avatar') }}">
<input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
<div class="mb-2"><input type="url" name="avatar_url" class="form-control form-control-sm" placeholder="رابط صورة البروفايل (URL)" value="{{ user.avatar_url or '' }}"></div>
<button type="submit" class="btn btn-outline-primary btn-sm w-100">تحديث صورة البروفايل</button>
</form>
</div>
</div>
...
<input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
<div class="mb-3">
<textarea name="content" class="form-control bg-dark text-light border-secondary" rows="3" placeholder="ماذا يدور في ذهنك؟" required maxlength="2000"></textarea>
</div>
<div class="mb-3">
<input type="url" name="image_url" class="form-control bg-dark text-light border-secondary" placeholder="رابط صورة (اختياري)">
</div>
<button type="submit" class="btn btn-primary"><i class="fas fa-paper-plane me-1"></i> نشر</button>
</form>
</div>
</div>
...
{% if c.avatar_url %}
<img src="{{ c.avatar_url }}" class="rounded-circle me-2" style="width:28px;height:28px;object-fit:cover;">
{% else %}
...
{% endfor %}
<form class="mt-2" onsubmit="addComment(event, '{{ post.id }}')">
<input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
<div class="input-group input-group-sm">
<input type="text" name="content" class="form-control bg-dark text-light border-secondary" placeholder="أضف تعليقاً..." required maxlength="500">
...
def update_avatar():
    ...
    if avatar_url:
        ...
        if parsed.scheme not in ('http', 'https') or not parsed.netloc:
            flash('رابط الصورة غير صالح', 'danger')
            return redirect(url_for('profile'))
    try:
        db.table('users').update({'avatar_url': avatar_url or None}).eq(
            'id', session['user_id']
        ).execute()

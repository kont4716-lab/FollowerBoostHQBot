...
<body>
    <nav class="navbar navbar-expand-lg navbar-ig sticky-top">
        <div class="container">
            <a class="navbar-brand" href="{{ url_for('index') }}"><i class="fas fa-coins me-1"></i>TaskCoins</a>
            ...
            <div class="collapse navbar-collapse" id="navMain">
                ...
                <a class="nav-link" href="{{ url_for('index') }}"><i class="fas fa-home me-1"></i>المهام</a>
                <a class="nav-link" href="{{ url_for('create_task') }}"><i class="fas fa-plus-circle me-1"></i>إنشاء</a>
                <a class="nav-link" href="{{ url_for('posts_feed') }}"><i class="fas fa-th-large me-1"></i>المنشورات</a>
                <a class="nav-link" href="{{ url_for('messages_inbox') }}">
                    <i class="fas fa-paper-plane me-1"></i>الرسائل
                    {% if unread_messages and unread_messages > 0 %}
                    <span class="badge-notif">{{ unread_messages if unread_messages < 100 else '99+' }}</span>
                    {% endif %}
                </a>
                <a class="nav-link" href="{{ url_for('profile') }}"><i class="fas fa-user me-1"></i>الملف</a>
                <a class="nav-link" href="{{ url_for('ai_chat') }}"><i class="fas fa-robot me-1"></i>AI</a>
...
{% else %}
<div class="p-4 text-center text-muted">لا توجد محادثات بعد</div>
{% endfor %}
</div>
</div>
</div>
...
def inject_globals():
    ...
    if session.get('user_id'):
        try:
            ...
            unread = 0
    return dict(csrf_token=generate_csrf, unread_messages=unread)

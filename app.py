...
# 15. بوت تيليجرام: /telegram/webhook يرد بـ Gemini عند أي رسالة نصية
#     + /telegram/set-webhook و /telegram/webhook-info لإدارة الربط
# ==========================================
...
if not TELEGRAM_BOT_TOKEN:
    ...
    print("تحذير: متغير البيئة TELEGRAM_BOT_TOKEN غير موجود أو فارغ!")
    print("لن يعمل Webhook تيليجرام بدون التوكن.")
    print("======================================")
else:
    print("========== TELEGRAM INFO ==========")
    print("تم العثور على TELEGRAM_BOT_TOKEN بنجاح.")
    print("===================================")
...
def telegram_send_message(chat_id, text: str, reply_to_message_id=None) -> bool:
    """إرسال رسالة نصية عبر Bot API. يعيد True عند النجاح."""
    ...
    try:
        ...
        log_action('error', f'telegram_send_message failed: {e}')
        return False


def telegram_ai_reply(user_text: str, first_name: str = '') -> str:
    """توليد رد Gemini لرسالة تيليجرام."""
    if not user_text:
        return 'مرحباً بك في TaskCoins Hub! أرسل أي رسالة وسأرد عليك.'
    if not gemini_client:
        return 'عذراً، خدمة الذكاء الاصطناعي غير مفعّلة حالياً على الخادم.'
    try:
        prompt = user_text
        if first_name:
            prompt = f'المستخدم اسمه {first_name}. رسالته: {user_text}'
        response = gemini_client.models.generate_content(
            model='gemini-3.6-flash',
            contents=prompt,
        )
        reply = (response.text or '').strip()
        return reply if reply else 'لم أستطع توليد رد. حاول مرة أخرى.'
    except Exception as e:
        log_action('error', f'telegram_ai_reply Gemini failed: {e}')
        return 'عذراً، حدث خطأ مؤقت أثناء توليد الرد. حاول لاحقاً.'
...
<body>
    <nav class="navbar navbar-expand-lg navbar-ig sticky-top">
        <div class="container">
            ...
            <!-- أيقونات رئيسية أفقية على الكمبيوتر فقط -->
            <div class="d-none d-lg-flex navbar-nav me-auto align-items-center">
                ...
                <a class="nav-link" href="{{ url_for('create_task') }}" title="إنشاء"><i class="fas fa-plus-circle me-1"></i>إنشاء</a>
            ...
            <!-- قائمة قصيرة للموبايل: فقط الإضافات -->
            <button class="navbar-toggler border-0 d-lg-none" type="button" data-bs-toggle="collapse" data-bs-target="#navMobileExtra">
                <i class="fas fa-ellipsis-v text-light"></i>
            </button>
            <div class="collapse navbar-collapse d-lg-none" id="navMobileExtra">
                <div class="navbar-nav">
                    <a class="nav-link" href="{{ url_for('create_task') }}"><i class="fas fa-plus-circle me-2"></i>إنشاء مهمة</a>
                    <a class="nav-link" href="{{ url_for('ai_chat') }}"><i class="fas fa-robot me-2"></i>الذكاء الاصطناعي</a>
                    {% if session.get('is_admin') %}
                    <a class="nav-link text-warning" href="{{ url_for('admin_dashboard') }}"><i class="fas fa-shield-alt me-2"></i>لوحة الإدارة</a>
                    {% endif %}
                    <a class="nav-link" href="{{ url_for('logout') }}"><i class="fas fa-sign-out-alt me-2"></i>تسجيل الخروج</a>
                </div>
            </div>
            {% endif %}
        </div>
    ...
    <nav class="bottom-nav">
        ...
        <a href="{{ url_for('profile') }}" title="الملف"><i class="fas fa-user-circle"></i></a>
    </nav>
    {% endif %}
</body>
</html>''',
...
# ==========================================
# Webhook تيليجرام (بوت يرد بـ Gemini)
# ==========================================
...
def telegram_webhook():
    """
    استقبال تحديثات تيليجرام (Update) والرد بالذكاء الاصطناعي.
    اربط الـ Webhook عبر:
      https://api.telegram.org/bot<TOKEN>/setWebhook?url=https://YOUR-APP.onrender.com/telegram/webhook
    """
    if not TELEGRAM_BOT_TOKEN:
    ...
    try:
        ...
        log_action('info', f"Telegram webhook update_id={update.get('update_id')}")

        # رسائل عادية (نص)
        message = update.get('message') or update.get('edited_message')
        if not message:
            # تجاهل أنواع أخرى (callback_query، channel_post، إلخ)
        ...
        first_name = from_user.get('first_name') or from_user.get('username') or ''

        # أوامر بسيطة
        if text.startswith('/start'):
            welcome = (
                f'أهلاً{" " + first_name if first_name else ""}! 👋\n'
                'أنا بوت TaskCoins Hub.\n'
                'أرسل أي رسالة وسأرد عليك بالذكاء الاصطناعي.'
            )
            telegram_send_message(chat_id, welcome, reply_to_message_id=message_id)
            return jsonify({'ok': True})
        ...
        if text.startswith('/help'):
            help_text = (
                ...
                '/start — بدء المحادثة\n'
                '/help — هذه الرسالة\n'
                'أي نص آخر → رد من Gemini AI'
            )
            telegram_send_message(chat_id, help_text, reply_to_message_id=message_id)
            return jsonify({'ok': True})

        if not text:
            telegram_send_message(
                chat_id,
                'أرسل رسالة نصية لأرد عليك.',
                reply_to_message_id=message_id,
            )
            return jsonify({'ok': True})

        # توليد الرد بالذكاء الاصطناعي وإرساله
        reply = telegram_ai_reply(text, first_name=first_name)
        telegram_send_message(chat_id, reply, reply_to_message_id=message_id)
        return jsonify({'ok': True})

    except Exception as e:
        log_action('error', f'Telegram webhook failed: {e}')
        # نرجع 200 حتى لا يعيد تيليجرام المحاولة بلا نهاية
...
def telegram_set_webhook():
    ...
    استخدم: GET/POST /telegram/set-webhook?url=https://YOUR-APP.onrender.com/telegram/webhook
    أو بدون url سيستخدم الطلب الحالي كأساس.
    """
    if not TELEGRAM_BOT_TOKEN:
        return jsonify({'ok': False, 'error': 'TELEGRAM_BOT_TOKEN missing'}), 503
    ...
    try:
        ...
        webhook_url = (request.args.get('url') or request.form.get('url') or '').strip()
        if not webhook_url:
            # بناء الرابط من الطلب الحالي
            base = request.url_root.rstrip('/')
            if base.startswith('http://') and 'onrender.com' in base:
                base = 'https://' + base[len('http://'):]
            webhook_url = f'{base}/telegram/webhook'

        payload = _json.dumps({'url': webhook_url}).encode('utf-8')
        req = urllib.request.Request(
        ...
        log_action('info', f'Telegram setWebhook -> {webhook_url}: {body}')
        return jsonify({'ok': body.get('ok', False), 'result': body, 'webhook_url': webhook_url})
    except Exception as e:
        log_action('error', f'telegram_set_webhook failed: {e}')
...
def telegram_webhook_info():
    """عرض معلومات الـ Webhook الحالي من تيليجرام."""
    if not TELEGRAM_BOT_TOKEN:
        return jsonify({'ok': False, 'error': 'TELEGRAM_BOT_TOKEN missing'}), 503
    try:
        import urllib.request
        ...
        with urllib.request.urlopen(
            f'{TELEGRAM_API_BASE}/getWebhookInfo', timeout=15
        ) as resp:
            body = _json.loads(resp.read().decode('utf-8'))
        return jsonify(body)
    except Exception as e:
...
def manychat_webhook():
    """
    Webhook لاستقبال رسائل ManyChat وإرجاع رد JSON مناسب.
    يدعم Dynamic Block (version v2) وحقول قابلة للتعيين عبر JSONPath.
    """
    try:
        data = request.get_json(silent=True) or {}
        log_action('info', f"ManyChat webhook received: keys={list(data.keys())}")

        # استخراج نص الرسالة من الحقول الشائعة في ManyChat
        user_message = (
            data.get('last_input_text')
            or data.get('message')
            or data.get('text')
            or data.get('last_text_input')
            or (data.get('content') if isinstance(data.get('content'), str) else None)
            or ''
        )
        if isinstance(user_message, dict):
            user_message = user_message.get('text') or user_message.get('message') or ''
        ...
        if user_message and gemini_client:
            try:
                prompt = user_message
                if first_name:
                    prompt = f"المستخدم اسمه {first_name}. رسالته: {user_message}"
                response = gemini_client.models.generate_content(
                    model='gemini-3.6-flash',
                    contents=prompt,
                )
                ...
                log_action('error', f"ManyChat Gemini failed: {e}")
                reply_text = None

        if not reply_text:
            if user_message:
                reply_text = f'تم استلام رسالتك: {user_message[:200]}'
            else:
                reply_text = 'مرحباً بك في TaskCoins Hub! كيف يمكنني مساعدتك؟'

        # صيغة Dynamic Block (v2) + حقول بسيطة للتعيين عبر JSONPath في External Request
        response_payload = {
            'version': 'v2',
            'content': {
                'messages': [
        ...
        log_action('error', f"ManyChat webhook failed: {e}")
        return jsonify({
            'version': 'v2',
            'content': {
                'messages': [
                    {
                        'type': 'text',
                        'text': 'عذراً، حدث خطأ مؤقت. حاول مرة أخرى لاحقاً.',
                    }
                ]
            },
            'reply': 'عذراً، حدث خطأ مؤقت. حاول مرة أخرى لاحقاً.',
            'message': 'عذراً، حدث خطأ مؤقت. حاول مرة أخرى لاحقاً.',
            'success': False,
